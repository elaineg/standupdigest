...
4. Toggle to a "Sprint Review" tab (Weekly Status stays the default; the same already-parsed CSV is reused) → using the same column auto-detect/manual-mapping/localStorage plus two added columns (Story Points/Estimate, Sprint/Cycle), a sprint selector defaulting to the most recent sprint drives an editable sprint-review block showing VELOCITY (points completed vs committed, plus issue counts), SCOPE CHANGE (points/issues added or removed mid-sprint, or an explicit "scope change unavailable" note when the data can't support it), SPILLOVER (issues still open at sprint end, reusing the carry-over logic), and BY ASSIGNEE (points + issue counts per person), copyable as Markdown or plaintext with the same copy-fidelity + "Copied ✓" contract.
